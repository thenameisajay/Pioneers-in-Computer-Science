...
router.post('/', async (req, res) => {
  ...
  if (includesRelatedPhrase) {
    ...
    try {
      ...
      return res.render(path.join(__dirname, '../views/chat.ejs'), { message: message, answer: answer });
    } catch (err) {
      console.log(err);
    }
  }
  ...
  try {
    const response = await openai.createCompletion({
      ...
      prompt: `The following is a conversation with an AI assistant specialized in pioneers of computer science AND i CAN ONLY STRICTLY TALK ABOUT COMPUTER SCIENCE AND PIONEERS OF COMPUTER SCIENCE. The assistant name is ECHO (Electronic Chatbot for Help and Operations ) is helpful, creative, clever, and very friendly.\n\nHuman: ${message}\nAI:`,
      temperature: 0.9,
      max_tokens: 150,
      top_p: 1,
      frequency_penalty: 0,
